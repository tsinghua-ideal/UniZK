#include "ramsim/src/base/base.h"
#include "ramsim/src/base/request.h"
#include "ramsim/src/base/config.h"
#include "ramsim/src/frontend/frontend.h"
#include "ramsim/src/memory_system/impl/generic_DRAM_system.cpp"
#include "ramsim/src/frontend/impl/ramsim/ramsim.h"
...
public:
    ...
    MemCpyAccel(std::string config_path) {
        ...
        rdHandler = [this](Ramulator::MemReq& req) { return this->handleRead(req); };
        wrHandler = [this](Ramulator::MemReq& req) { return this->handleWrite(req); };
    ...
    void start(Ramulator::Addr_t src, Ramulator::Addr_t dst, uint32_t size) {
        for (Ramulator::Addr_t offset = 0; offset < size; offset += accSize) {
            Ramulator::MemReq rd{id, src + offset, RAMSIM_READREQ, 0, accSize, std::vector<uint64_t>()};
            id += 1;
            ...
            Ramulator::MemReq wr{id, dst + offset, RAMSIM_WRITEREQ, 0, accSize, std::vector<uint64_t>(1, id-1)};
            id += 1;
...
int main(int argc, char** argv) {
    ...
    accel.start(0x10000000, 0x20000000, 4096);
