    ...
    StarkProofWithPublicInputs<F, C, D>
{
    ...
    #[inline]
    fn write_starky_proof_with_public_inputs(&self, buffer: &mut Vec<u8>) -> IoResult<()>
    where
        F: RichField + Extendable<D>,
        C: GenericConfig<D, F = F>,
    {
        ...
        // buffer.write_proof(proof)?;
        buffer.write_merkle_cap(&proof.trace_cap)?;
        if let Some(permutation_zs_cap) = &proof.permutation_zs_cap {
            buffer.write_merkle_cap(permutation_zs_cap)?;
        }
        buffer.write_merkle_cap(&proof.quotient_polys_cap)?;
        // buffer.write_opening_set(&proof.openings);
        buffer.write_field_ext_vec::<F, D>(&proof.openings.local_values)?;
        buffer.write_field_ext_vec::<F, D>(&proof.openings.next_values)?;
        if let Some(permutation_zs) = &proof.openings.permutation_zs {
            buffer.write_field_ext_vec::<F, D>(permutation_zs)?;
        }
        if let Some(permutation_zs_next) = &proof.openings.permutation_zs_next {
            buffer.write_field_ext_vec::<F, D>(permutation_zs_next)?;
        }
        buffer.write_field_ext_vec::<F, D>(&proof.openings.quotient_polys)?;

        buffer.write_fri_proof::<F, C, D>(&proof.opening_proof)?;

        buffer.write_usize(public_inputs.len())?;
        buffer.write_field_vec(public_inputs)
    }

    pub fn to_bytes(&self) -> Vec<u8> {
        let mut buffer = Vec::new();
        self.write_starky_proof_with_public_inputs(&mut buffer)
